...
class GuildAttendance(commands.Cog):
    ...
    async def get_event_from_cache(
        ...
        try:
            ...
            logging.error(
                f"[GuildAttendance] Error retrieving event {event_id} for guild {guild_id}: {e}",
                exc_info=True,
            )
            return None
    ...
    async def set_event_in_cache(
        ...
        try:
            ...
            logging.error(
                f"[GuildAttendance] Error storing event {event_id} for guild {guild_id}: {e}",
                exc_info=True,
            )
    ...
    async def get_closed_events_for_guild(self, guild_id: int) -> List[Dict]:
        ...
        try:
            ...
            logging.error(
                f"[GuildAttendance] Error retrieving closed events for guild {guild_id}: {e}",
                exc_info=True,
            )
            return []
    ...
    async def get_guild_settings(self, guild_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            logging.error(
                f"[GuildAttendance] Error getting guild settings for {guild_id}: {e}",
                exc_info=True,
            )
            return {}
    ...
    async def get_guild_members(self, guild_id: int) -> Dict[int, Dict[str, Any]]:
        ...
        try:
            ...
            logging.error(
                f"[GuildAttendance] Error getting guild members for {guild_id}: {e}",
                exc_info=True,
            )
            return {}
    ...
    async def _update_centralized_cache(
        ...
        try:
            ...
            logging.debug(
                f"[GuildAttendance] Updated centralized cache for {len(guild_members)} members in guild {guild_id}"
            )
        except Exception as e:
            logging.error(
                f"[GuildAttendance] Error updating centralized cache: {e}",
                exc_info=True,
            )
    ...
    async def process_event_registrations(
        ...
        """
        logging.info(
            f"[GuildAttendance] Processing registrations for event {event_id} in guild {guild_id}"
        )
        ...
        if isinstance(registrations_raw, str):
            try:
                ...
                logging.warning(
                    f"[GuildAttendance] Failed to parse registrations JSON for event {event_id}: {e}"
                )
                registrations = {}
        else:
        ...
        for member_id in all_registered:
            if member_id not in guild_members:
                initial_nb_events = (
                    1 if await self._member_has_members_role(guild, member_id) else 0
                )
            ...
            if dkp_registration > 0:
                ...
                logging.debug(
                    f"[GuildAttendance] Member {member_id} earned {dkp_registration} DKP for registration"
                )
        ...
        if updates_to_batch:
            try:
                ...
                logging.info(
                    f"[GuildAttendance] Updated registration stats for {len(updates_to_batch)} members in event {event_id}"
                )
                ...
                logging.error(
                    f"[GuildAttendance] Error updating registration stats: {e}",
                    exc_info=True,
                )
    ...
    async def check_voice_presence(self):
        ...
        try:
            ...
            if guild_tasks:
                ...
                logging.debug(
                    f"[GuildAttendance] Completed processing {len(guild_tasks)} guilds"
                )

        except Exception as e:
            logging.error(
                f"[GuildAttendance] Error in voice presence check: {e}", exc_info=True
            )
    ...
    async def _get_current_events_for_guild(
        ...
        logging.debug(
            f"[GuildAttendance] Filtering events for guild {guild_id} at {now}"
        )
        ...
        try:
            ...
            for row in rows:
                try:
                    ...
                    logging.warning(
                        f"[GuildAttendance] Invalid event data for {row[0]}_{row[1]}: {e}"
                    )
                    continue
                ...
                try:
                    ...
                    if isinstance(event_data["event_time"], str):
                        ...
                        logging.warning(
                            f"[GuildAttendance] Unknown time type for event {event_data['event_id']}: {type(event_data['event_time'])}"
                        )
                    ...
                    logging.debug(
                        f"[GuildAttendance] Event {event_data['event_id']}: start={event_start}, check_time={event_check_time}, end={event_end}, check_end={event_check_end}, now={now}"
                    )
                    logging.debug(
                        f"[GuildAttendance] Event {event_data['event_id']}: condition {event_check_time} <= {now} <= {event_check_end} = {event_check_time <= now <= event_check_end}"
                    )

                    if event_check_time <= now <= event_check_end:
                        current_events.append(event_data)
                        logging.debug(
                            f"[GuildAttendance] Event {event_data['event_id']} matches time window, added to current events"
                        )

                except Exception as e:
                    logging.error(
                        f"[GuildAttendance] Error parsing event time for {event_data['event_id']}: {e}"
                    )
                    continue

        except Exception as e:
            logging.error(
                f"[GuildAttendance] Error loading events for guild {guild_id}: {e}",
                exc_info=True,
            )

        logging.debug(
            f"[GuildAttendance] Returning {len(current_events)} events for guild {guild_id}"
        )
        return current_events

    async def _process_voice_attendance(
        self, guild: discord.Guild, event_data: Dict, now: datetime
    ) -> None:
        """
        ...
        logging.debug(
            f"[GuildAttendance] Processing voice attendance for event {event_id}"
        )
        ...
        try:
            ...
            logging.debug(
                f"[GuildAttendance] Found {len(voice_members)} members in voice channels"
            )
            ...
            try:
                ...
                logging.error(
                    f"[GuildAttendance] Error parsing DKP values for event {event_id}: {e}"
                )
                dkp_presence = 0
                dkp_registration = 0

            logging.debug(
                f"[GuildAttendance] Event {event_id}: dkp_presence={dkp_presence}, dkp_registration={dkp_registration}"
            )
            ...
            logging.debug(
                f"[GuildAttendance] Event {event_id}: {len(presence_ids)} present, {len(tentative_ids)} tentative, {len(absence_ids)} absent registrations"
            )
            ...
            logging.debug(
                f"[GuildAttendance] Event {event_id}: calculated {len(attendance_changes)} attendance changes"
            )

            if attendance_changes:
                logging.info(
                    f"[GuildAttendance] Applying {len(attendance_changes)} attendance changes for event {event_id}"
                )
                await self._apply_attendance_changes(
                    guild.id, event_id, attendance_changes
                )
                await self._update_event_actual_presence(
                    guild.id, event_id, list(voice_members)
                )
                ...
                logging.debug(
                    f"[GuildAttendance] Marked event {event_id} as processed - will never be processed again"
                )
            else:
                logging.debug(
                    f"[GuildAttendance] No attendance changes to apply for event {event_id}"
                )

        except Exception as e:
            logging.error(
                f"[GuildAttendance] Error processing voice attendance for event {event_id}: {e}",
                exc_info=True,
            )
    ...
    def _was_already_processed(self, event_data: Dict) -> bool:
        ...
        if event_id in self._processed_events:
            logging.debug(
                f"[GuildAttendance] Event {event_id} was already processed for attendance, skipping"
            )
            return True

        return False

    async def _member_has_members_role(
        ...
        try:
            ...
            logging.error(
                f"[GuildAttendance] Error checking member role for {member_id}: {e}"
            )
            return False
        ...
        for member_id in all_registered:
            ...
            if member_id in presence_ids:
                if is_voice_present:
                    ...
                    change["reason"] = GUILD_ATTENDANCE["reasons"][
                        "present_and_present"
                    ].get(
                        guild_lang,
                        GUILD_ATTENDANCE["reasons"]["present_and_present"].get("en-US"),
                    )
                else:
                    ...
                    change["reason"] = GUILD_ATTENDANCE["reasons"][
                        ...
                        guild_lang,
                        GUILD_ATTENDANCE["reasons"]["present_but_absent"].get("en-US"),
                    )
                ...
                if is_voice_present:
                    ...
                    change["attendance_change"] = 1
                    change["reason"] = GUILD_ATTENDANCE["reasons"][
                        "tentative_and_present"
                    ].get(
                        guild_lang,
                        GUILD_ATTENDANCE["reasons"]["tentative_and_present"].get(
                            "en-US"
                        ),
                    )
                else:
                    change["reason"] = GUILD_ATTENDANCE["reasons"][
                        "tentative_and_absent"
                    ].get(
                        guild_lang,
                        GUILD_ATTENDANCE["reasons"]["tentative_and_absent"].get(
                            "en-US"
                        ),
                    )
                ...
                if is_voice_present:
                    ...
                    change["reason"] = GUILD_ATTENDANCE["reasons"][
                        ...
                        guild_lang,
                        GUILD_ATTENDANCE["reasons"]["absent_but_present"].get("en-US"),
                    )
                else:
                    change["reason"] = GUILD_ATTENDANCE["reasons"][
                        "absent_and_absent"
                        ...
                        guild_lang,
                        GUILD_ATTENDANCE["reasons"]["absent_and_absent"].get("en-US"),
                    )

            if change["dkp_change"] != 0 or change["attendance_change"] != 0:
                changes.append(change)
                logging.debug(
                    f"[GuildAttendance] Added change for member {member_id}: dkp_change={change['dkp_change']}, attendance_change={change['attendance_change']}, reason={change['reason']}"
                )
            else:
                logging.debug(
                    f"[GuildAttendance] No change for member {member_id}: dkp_change={change['dkp_change']}, attendance_change={change['attendance_change']}"
                )
    ...
    async def _apply_attendance_changes(
        self, guild_id: int, event_id: int, changes: List[Dict]
    ) -> None:
        """
        ...
        if updates_to_batch:
            try:
                ...
                logging.info(
                    f"[GuildAttendance] Applied attendance changes for {len(updates_to_batch)} members in event {event_id}"
                )
                ...
                logging.error(
                    f"[GuildAttendance] Error applying attendance changes: {e}",
                    exc_info=True,
                )
    ...
    async def _update_event_actual_presence(
        ...
        try:
            ...
            if event_data:
                ...
                logging.debug(
                    f"[GuildAttendance] Updated actual_presence in cache for event {event_id}"
                )

        except Exception as e:
            logging.error(
                f"[GuildAttendance] Error updating actual presence for event {event_id}: {e}"
            )
    ...
    async def _send_registration_notification(
        ...
        try:
            ...
            title = GUILD_ATTENDANCE["notifications"]["registration"]["title"].get(
                guild_lang,
                GUILD_ATTENDANCE["notifications"]["registration"]["title"].get("en-US"),
            )
            description = (
                GUILD_ATTENDANCE["notifications"]["registration"]["description"]
                .get(
                    guild_lang,
                    GUILD_ATTENDANCE["notifications"]["registration"][
                        "description"
                    ].get("en-US"),
                )
            ...
            total_field = GUILD_ATTENDANCE["notifications"]["registration"][
                "total_registered"
            ].get(
                guild_lang,
                GUILD_ATTENDANCE["notifications"]["registration"][
            ...
            members_text = GUILD_ATTENDANCE["notifications"]["attendance"][
                "members"
            ].get(
                guild_lang,
                GUILD_ATTENDANCE["notifications"]["attendance"]["members"].get("en-US"),
            )
            dkp_total_text = GUILD_ATTENDANCE["notifications"]["attendance"][
                "dkp_total"
            ].get(
                guild_lang,
                GUILD_ATTENDANCE["notifications"]["attendance"]["dkp_total"].get(
                    "en-US"
                ),
            )
            embed.add_field(
            ...
            present_field = GUILD_ATTENDANCE["notifications"]["registration"][
                "present"
            ].get(
                guild_lang,
                GUILD_ATTENDANCE["notifications"]["registration"]["present"].get(
                    "en-US"
                ),
            )
            tentative_field = GUILD_ATTENDANCE["notifications"]["registration"][
                "tentative"
            ].get(
                guild_lang,
                GUILD_ATTENDANCE["notifications"]["registration"]["tentative"].get(
                    "en-US"
                ),
            )
            absent_field = GUILD_ATTENDANCE["notifications"]["registration"][
                "absent"
            ].get(
                guild_lang,
                GUILD_ATTENDANCE["notifications"]["registration"]["absent"].get(
                    "en-US"
                ),
            )
            ...
            date_field = GUILD_ATTENDANCE["notifications"]["attendance"]["date"].get(
                guild_lang,
                GUILD_ATTENDANCE["notifications"]["attendance"]["date"].get("en-US"),
            )
            ...
            logging.error(
                f"[GuildAttendance] Error sending registration notification: {e}"
            )
    ...
    async def _send_attendance_notification(
        ...
        """
        logging.debug(
            f"[GuildAttendance] Attempting to send attendance notification for event {event_id} with {len(changes)} changes"
        )

        settings = await self.get_guild_settings(guild_id)
        if not settings:
            logging.warning(
                f"[GuildAttendance] No settings found for guild {guild_id}, cannot send notification"
            )
            return
        if not settings.get("notifications_channel"):
            logging.warning(
                f"[GuildAttendance] No notifications channel configured for guild {guild_id}, cannot send notification"
            )
            return
        if not changes:
            logging.debug(
                f"[GuildAttendance] No changes to report for event {event_id}, skipping notification"
            )
            return
        ...
        try:
            ...
            title = GUILD_ATTENDANCE["notifications"]["attendance"]["title"].get(
                guild_lang,
                GUILD_ATTENDANCE["notifications"]["attendance"]["title"].get("en-US"),
            )
            description = (
                GUILD_ATTENDANCE["notifications"]["attendance"]["description"]
                .get(
                    guild_lang,
                    GUILD_ATTENDANCE["notifications"]["attendance"]["description"].get(
                        "en-US"
                    ),
                )
                .format(event_id=event_id)
            ...
            dkp_modifications_title = GUILD_ATTENDANCE["notifications"]["attendance"][
                "dkp_modifications_summary"
            ].get(
                guild_lang,
                GUILD_ATTENDANCE["notifications"]["attendance"][
                    "dkp_modifications_summary"
                ].get("en-US"),
            )
            confirmed_presences_text = GUILD_ATTENDANCE["notifications"]["attendance"][
                "confirmed_presences"
            ].get(
                guild_lang,
                GUILD_ATTENDANCE["notifications"]["attendance"][
            ...
            impacted_members_text = GUILD_ATTENDANCE["notifications"]["attendance"][
                "impacted_members"
            ].get(
                guild_lang,
                GUILD_ATTENDANCE["notifications"]["attendance"]["impacted_members"].get(
                    "en-US"
                ),
            )
            embed.add_field(
            ...
            details_field = GUILD_ATTENDANCE["notifications"]["attendance"][
                "details"
            ].get(
                guild_lang,
                GUILD_ATTENDANCE["notifications"]["attendance"]["details"].get("en-US"),
            )
            ...
            if len(changes) <= 10:
                ...
                too_many_msg = (
                    GUILD_ATTENDANCE["notifications"]["attendance"]["too_many_changes"]
                    .get(
                        guild_lang,
                        GUILD_ATTENDANCE["notifications"]["attendance"][
                            "too_many_changes"
            ...
            await channel.send(embed=embed)
            logging.info(
                f"[GuildAttendance] Successfully sent attendance notification for event {event_id} to channel {settings['notifications_channel']}"
            )

        except Exception as e:
            logging.error(
                f"[GuildAttendance] Error sending attendance notification: {e}",
                exc_info=True,
            )
    ...
    async def _process_guild_attendance(self, guild: discord.Guild, now: datetime):
        ...
        try:
            ...
            logging.debug(
                f"[GuildAttendance] Processing guild {guild_id} ({guild.name})"
            )
            current_events = await self._get_current_events_for_guild(guild_id, now)
            logging.debug(
                f"[GuildAttendance] Found {len(current_events)} current events for guild {guild_id}"
            )

            for event_data in current_events:
                logging.info(
                    f"[GuildAttendance] Processing voice attendance for event {event_data['event_id']} at {now}"
                )
                try:
                    await self._process_voice_attendance(guild, event_data, now)
                except Exception as e:
                    logging.error(
                        f"[GuildAttendance] Error processing voice attendance for event {event_data.get('event_id')}: {e}"
                    )

            logging.debug(
                f"[GuildAttendance] Completed processing {len(current_events)} events for guild {guild_id}"
            )

        except Exception as e:
            logging.error(
                f"[GuildAttendance] Error processing guild {guild.id}: {e}",
                exc_info=True,
            )
