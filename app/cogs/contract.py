...
async def save_contract_message(bot, guild_id, message_id):
    ...
    try:
        ...
        logging.error(
            f"[ContractManager] Error saving contract message for guild {guild_id}: {e}",
            exc_info=True,
        )
        raise
...
async def load_contract_message(bot, guild_id):
    ...
    try:
        ...
        logging.error(
            f"[ContractManager] Error loading contract message for guild {guild_id}: {e}",
            exc_info=True,
        )
        return None
...
async def delete_contract_message(bot, guild_id):
    ...
    try:
        ...
        logging.error(
            f"[ContractManager] Error deleting contract message for guild {guild_id}: {e}",
            exc_info=True,
        )
        raise
...
class ContractSelect(discord.ui.View):
    ...
    def __init__(self, bot, author, guild_lang):
        ...
        if options_monster:
            ...
            logging.error(
                "[ContractManager] No options found for 'monster_elimination' in translations."
            )
        ...
        if options_dynamic:
            ...
            logging.error(
                "[ContractManager] No options found for 'dynamic_events' in translations."
            )
        ...
        if options_dungeon:
            ...
            logging.error(
                "[ContractManager] No options found for 'open_dungeon' in translations."
            )
        ...
        async def validate_callback(interaction: discord.Interaction):
            ...
            try:
                ...
                logging.error(
                    f"[ContractManager] Error deleting interactive message: {e}",
                    exc_info=True,
                )
    ...
    async def _get_channel_safe(self, channel_id: int) -> Optional[discord.TextChannel]:
        ...
        if not channel:
            try:
                ...
                logging.error(
                    f"[ContractManager] Error fetching channel {channel_id}: {e}"
                )
                return None
        return channel
    ...
    async def post_event_message(self, interaction: discord.Interaction):
        ...
        try:
            ...
            try:
                ...
                logging.debug(
                    f"[ContractManager] Contract message saved for guild {guild_id} with message ID {event_message.id}."
                )
            ...
            logging.error(
                f"[ContractManager] Failed to post or save contract message for guild {guild_id}: {e}",
                exc_info=True,
            )
            error_msg = (
                STAFF_TOOLS.get("contract_data", {})
...
class Contract(commands.Cog):
    ...
    async def contract_delete(self, ctx: discord.ApplicationContext):
        ...
        try:
            ...
            if message_id:
                try:
                    ...
                    try:
                        ...
                        logging.error(
                            f"[ContractManager] DB cleanup failed after message deletion: {db_error}"
                        )
            ...
            logging.error(
                f"[ContractManager] Error in /contract_delete command: {e}",
                exc_info=True,
            )
            try:
                settings = await self._get_guild_settings(ctx.guild.id)
    ...
    async def contract_delete_cron(self):
        ...
        for guild in self.bot.guilds:
            try:
                ...
                if not channel_id:
                    logging.debug(
                        f"[ContractManager] No event channel configured for guild {guild_id}"
                    )
                    continue
                ...
                if not channel:
                    logging.warning(
                        f"[ContractManager] Event channel {channel_id} not accessible for guild {guild_id}"
                    )
                    continue
                ...
                if message_id:
                    try:
                        ...
                        try:
                            ...
                            logging.error(
                                f"[ContractManager] DB cleanup failed for guild {guild_id}: {db_error}"
                            )
                        logging.info(
                            f"[ContractManager] Contract deleted for guild {guild_id}"
                        )
                        processed += 1
                    except discord.NotFound:
                        try:
                            ...
                            pass
                        logging.debug(
                            f"[ContractManager] Contract already deleted for guild {guild_id}"
                        )
                ...
                logging.error(
                    f"[ContractManager] Error processing guild {guild_id} in cron: {e}",
                    exc_info=True,
                )
                continue
        ...
        if failed_guilds:
            logging.warning(
                f"[ContractManager] Cron failed for {len(failed_guilds)} guilds: {failed_guilds}"
            )

        logging.info(
            f"[ContractManager] Cron completed: {processed} contracts processed, {len(failed_guilds)} failures"
        )
