...
class DynamicVoice(commands.Cog):
    ...
    def get_safe_username(self, member):
        ...
        """
        return (
            f"User#{member.discriminator}"
            if hasattr(member, "discriminator")
            else f"User{member.id}"
        )
    ...
    async def on_ready(self):
        ...
        logging.debug(
            "[DynamicVoice] 'load_persistent_channels' task started from on_ready"
        )
    ...
    async def load_persistent_channels(self):
        ...
        try:
            ...
            await self.bot.cache.set(
                "temporary", dynamic_channels, "dynamic_voice_channels"
            )
            logging.debug(
                f"[DynamicVoice] Persistent dynamic channels loaded from DB: {dynamic_channels}"
            )
        except Exception as e:
            logging.error(
                f"[DynamicVoice] Error loading persistent channels: {e}", exc_info=True
            )
    ...
    async def on_voice_state_update(
        ...
        create_room_channel = (
            channels_data.get("create_room_channel") if channels_data else None
        )
        monitored_channels = {create_room_channel} if create_room_channel else set()
        safe_name = self.get_safe_username(member)
        logging.debug(
            f"[DynamicVoice] on_voice_state_update: {safe_name} - Before: {before.channel.id if before.channel else None}, After: {after.channel.id if after.channel else None}"
        )
        logging.debug(
            f"[DynamicVoice] Monitored channels for guild {guild.id}: {monitored_channels}"
        )
        ...
        if after.channel and after.channel.id in monitored_channels:
            safe_name = self.get_safe_username(member)
            logging.debug(
                f"[DynamicVoice] {safe_name} joined monitored channel {after.channel.id}. Preparing to create a temporary channel."
            )

            user_channels = (
                await self.bot.cache.get("temporary", f"user_channels_{member.id}")
                or []
            )
            valid_user_channels = []
            ...
            if len(valid_user_channels) != len(user_channels):
                if valid_user_channels:
                    await self.bot.cache.set(
                        "temporary", valid_user_channels, f"user_channels_{member.id}"
                    )
                else:
                    await self.bot.cache.delete(
                        "temporary", f"user_channels_{member.id}"
                    )

            user_channel_count = len(valid_user_channels)
            if user_channel_count >= self.max_channels_per_user:
                logging.warning(
                    f"[DynamicVoice] {safe_name} reached max channels limit ({self.max_channels_per_user})"
                )
                return

            now = time.time()
            last_action = (
                await self.bot.cache.get("temporary", f"cooldown_{member.id}") or 0
            )

            reduced_cooldown = False
            if before.channel:
                dynamic_channels = (
                    await self.bot.cache.get("temporary", "dynamic_voice_channels")
                    or set()
                )
                if (
                    before.channel.id in dynamic_channels
                    and before.channel.id in valid_user_channels
                ):
                    reduced_cooldown = True
                    logging.debug(
                        f"[DynamicVoice] {safe_name} left their own channel, reducing cooldown"
                    )

            cooldown_time = 0 if reduced_cooldown else self.cool_down_seconds
            if now - last_action < cooldown_time:
                logging.warning(
                    f"[DynamicVoice] {safe_name} is on cool down ({cooldown_time}s)"
                )
                return
            ...
            guild_lang = (
                await self.bot.cache.get_guild_data(guild.id, "guild_lang") or "en-US"
            )
            ...
            role_absent_members_id = (
                roles_data.get("absent_members") if roles_data else None
            )

            room_name_template = DYNAMIC_VOICE.get("channel_name", {}).get(
                guild_lang,
                DYNAMIC_VOICE.get("channel_name", {}).get(
                    "en-US", "Channel of {username}"
                ),
            )
            try:
                ...
                logging.error(
                    f"[DynamicVoice] Error formatting channel name: {e}", exc_info=True
                )
                channel_name = f"Channel of {member.display_name}"

            channel_name = self.sanitize_channel_name(channel_name)
            logging.debug(
                f"[DynamicVoice] Channel name obtained from JSON: '{channel_name}'"
            )
            ...
            if role_members_id:
                ...
                if role_members:
                    overwrites[role_members] = discord.PermissionOverwrite(
                        view_channel=True, connect=True
                    )
            if role_absent_members_id:
                role_absent = guild.get_role(role_absent_members_id)
                if role_absent:
                    overwrites[role_absent] = discord.PermissionOverwrite(
                        view_channel=True, connect=True
                    )
            try:
                await asyncio.sleep(0.2)
                logging.debug(
                    f"[DynamicVoice] Attempting to create a temporary channel for {safe_name} with name '{channel_name}'"
                )
                ...
                logging.debug(
                    f"[DynamicVoice] Temporary channel created: {new_channel.name} (ID: {new_channel.id})"
                )
                try:
                    await member.move_to(new_channel)
                except discord.HTTPException as e:
                    logging.error(
                        f"[DynamicVoice] Failed to move {safe_name} to new channel: {e}",
                        exc_info=True,
                    )
                except Exception as e:
                    logging.error(
                        f"[DynamicVoice] Unexpected error moving {safe_name}: {e}",
                        exc_info=True,
                    )
                dynamic_channels = (
                    await self.bot.cache.get("temporary", "dynamic_voice_channels")
                ...
                await self.bot.cache.set(
                    "temporary", user_channels, f"user_channels_{member.id}"
                )
                try:
                    query_insert = "INSERT INTO dynamic_voice_channels (channel_id, guild_id) VALUES (%s, %s)"
                    await self.bot.run_db_query(
                        query_insert, (new_channel.id, guild.id), commit=True
                    )
                    logging.debug(
                        f"[DynamicVoice] Persistent channel registered in DB for {safe_name}"
                    )
                except Exception as e:
                    logging.error(
                        f"[DynamicVoice] Error saving channel {new_channel.id} to DB: {e}",
                        exc_info=True,
                    )
            except asyncio.TimeoutError:
                logging.error(
                    f"[DynamicVoice] Timeout while creating channel for {safe_name}",
                    exc_info=True,
                )
            except Exception as e:
                logging.error(
                    f"[DynamicVoice] Error while creating channel for {safe_name}: {e}",
                    exc_info=True,
                )

        dynamic_channels = (
            await self.bot.cache.get("temporary", "dynamic_voice_channels") or set()
        )
            ...
            if len(channel.members) == 0:
                try:
                    ...
                    for guild_member in guild.members:
                        user_channels = (
                            await self.bot.cache.get(
                                "temporary", f"user_channels_{guild_member.id}"
                            )
                            or []
                        )
                        if channel.id in user_channels:
                            ...
                            if user_channels:
                                await self.bot.cache.set(
                                    "temporary",
                                    user_channels,
                                    f"user_channels_{guild_member.id}",
                                )
                            else:
                                await self.bot.cache.delete(
                                    "temporary", f"user_channels_{guild_member.id}"
                    ...
                    logging.debug(
                        f"[DynamicVoice] Voice channel deleted: {channel.name} (ID: {channel.id})"
                    )

                    try:
                        query_delete = (
                            "DELETE FROM dynamic_voice_channels WHERE channel_id = %s"
                        )
                        await self.bot.run_db_query(
                            query_delete, (channel.id,), commit=True
                        )
                        logging.debug(
                            f"[DynamicVoice] Database record removed for channel {channel.id}"
                        )
                    except Exception as e:
                        logging.error(
                            f"[DynamicVoice] Error removing channel {channel.id} from DB: {e}",
                            exc_info=True,
                        )

                except discord.Forbidden:
                    logging.error(
                        f"[DynamicVoice] Insufficient permissions to delete channel {channel.name}.",
                        exc_info=True,
                    )
                except Exception as e:
                    logging.error(
                        f"[DynamicVoice] Error while deleting channel {channel.name}: {e}",
                        exc_info=True,
                    )
