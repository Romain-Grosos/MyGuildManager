...
logging.debug(
    f"[AbsenceManager] Loaded ABSENCE_TRANSLATIONS keys: {list(ABSENCE_TRANSLATIONS.keys())}"
)
if "title" in ABSENCE_TRANSLATIONS:
    logging.debug(
        f"[AbsenceManager] Title translation keys: {list(ABSENCE_TRANSLATIONS['title'].keys())}"
    )
else:
    logging.error(f"[AbsenceManager] No 'title' key in ABSENCE_TRANSLATIONS!")
    logging.debug(
        f"[AbsenceManager] Full absence_system structure: {list(global_translations.get('absence_system', {}).keys())}"
    )
...
class AbsenceManager(commands.Cog):
    ...
    def _register_absence_commands(self):
        ...
        if hasattr(self.bot, "absence_group"):
            self.bot.absence_group.command(
                name=ABSENCE_TRANSLATIONS.get("absence_add", {})
                .get("name", {})
                .get("en-US", "absence_add"),
                description=ABSENCE_TRANSLATIONS.get("absence_add", {})
                .get("description", {})
                .get("en-US", "Mark a member as absent."),
                name_localizations=ABSENCE_TRANSLATIONS.get("absence_add", {}).get(
                    "name", {}
                ),
                description_localizations=ABSENCE_TRANSLATIONS.get(
                    "absence_add", {}
                ).get("description", {}),
            )(self.absence_add)
        ...
        if role_absent and role_member:
            if role_member in member.roles:
                try:
                    ...
                    logging.error(
                        f"[AbsenceManager] Error removing member role from {member.name}: {e}"
                    )
            if role_absent not in member.roles:
                try:
                    await member.add_roles(role_absent)
                    logging.debug(
                        f"[AbsenceManager] Absent Members role assigned to {member.name} in guild {guild.id}"
                    )
                    ...
                    try:
                        ...
                        logging.error(
                            "[AbsenceManager] Error saving absence message: %s", e
                        )
                    ...
                    await self.notify_absence(
                        member,
                        "addition",
                        channels.get("forum_members_channel"),
                        guild_lang,
                    )
                except Exception as e:
                    logging.error(
                        f"[AbsenceManager] Error adding absent role to {member.name}: {e}"
                    )
    ...
    async def on_raw_message_delete(self, payload: discord.RawMessageDeleteEvent):
        ...
        if not member:
            logging.debug(
                "[AbsenceManager] Member %s not found in guild %s", member_id, guild.id
            )
            return
        ...
        try:
            row = await self.bot.run_db_query(
                ...
                (payload.guild_id, member_id),
                fetch_one=True,
            )
        except Exception as e:
            logging.error(
                "[AbsenceManager] Error checking remaining absence messages: %s", e
            )
            return
        if row and row[0] > 0:
            logging.debug(
                "[AbsenceManager] Other absence messages remain for %s, keeping 'absent' role.",
                member.name,
            )
            return
        ...
        if role_member and role_member not in member.roles:
            try:
                ...
                guild_lang = (
                    await self.bot.cache.get_guild_data(member.guild.id, "guild_lang")
                    or "en-US"
                )
                await self.notify_absence(
    ...
    async def _set_absent(
        ...
        if not cfg:
            logging.error(
                "[AbsenceManager] Guild config not found for guild %s", guild.id
            )
            return
        guild_lang = (
            await self.bot.cache.get_guild_data(member.guild.id, "guild_lang")
    ...
    async def absence_add(
        ...
        if not cfg:
            msg = await get_user_message(
                ctx, global_translations.get("absence_system", {}), "error_chan"
            )
        ...
        guild_lang = (
            await self.bot.cache.get_guild_data(ctx.guild.id, "guild_lang") or "en-US"
        )
        logging.debug(
            f"[AbsenceManager] Guild lang: {guild_lang}, Available translations: {list(ABSENCE_TRANSLATIONS.get('away_ok', {}).keys())}"
        )
        reason_text = (
            ABSENCE_TRANSLATIONS.get("away_ok", {})
            .get(
                guild_lang,
                ABSENCE_TRANSLATIONS.get("away_ok", {}).get(
                    "en-US", "**{member}** is now marked as away."
                ),
            )
        ...
        if return_date:
            back_text = (
                ABSENCE_TRANSLATIONS.get("back_time", {})
                .get(
                    guild_lang,
                    ABSENCE_TRANSLATIONS.get("back_time", {}).get(
                        "en-US", "Expected return: {return_date}"
                    ),
                )
                .format(return_date=return_date)
            )
            reason_text = f"{reason_text} {back_text}"

        await self._set_absent(ctx.guild, member, abs_chan, reason_text)
        resp = await get_user_message(
            ctx,
    ...
    async def notify_absence(
        ...
        if not channel:
            try:
                ...
                logging.error(
                    f"[AbsenceManager] Error fetching notification channel: {e}"
                )
                return

        if not channel:
            logging.error(
                "[AbsenceManager] Failed to access the members notification channel."
            )
            return
        ...
        logging.debug(
            f"[AbsenceManager] Embed guild_lang: {guild_lang}, Available title translations: {list(ABSENCE_TRANSLATIONS.get('title', {}).keys())}"
        )
        title = ABSENCE_TRANSLATIONS.get("title", {}).get(
            guild_lang,
            ABSENCE_TRANSLATIONS.get("title", {}).get(
                "en-US", "📢 Member status update"
            ),
        )
        logging.debug(f"[AbsenceManager] Retrieved title: {title}")
        member_label = ABSENCE_TRANSLATIONS.get("member_label", {}).get(
            guild_lang,
            ABSENCE_TRANSLATIONS.get("member_label", {}).get("en-US", "Member"),
        )
        status_label = ABSENCE_TRANSLATIONS.get("status_label", {}).get(
            guild_lang,
            ABSENCE_TRANSLATIONS.get("status_label", {}).get("en-US", "Status"),
        )
        absent_text = ABSENCE_TRANSLATIONS.get("absent", {}).get(
            guild_lang, ABSENCE_TRANSLATIONS.get("absent", {}).get("en-US", "🚨 Absent")
        )
        returned_text = ABSENCE_TRANSLATIONS.get("returned", {}).get(
            guild_lang,
            ABSENCE_TRANSLATIONS.get("returned", {}).get("en-US", "✅ Returned"),
        )
        ...
        try:
            ...
            logging.debug(
                f"[AbsenceManager] Notification sent for {member.name} ({status_text})."
            )
        except Exception as e:
            logging.error(
                f"[AbsenceManager] Error sending notification for {member.name}: {e}"
            )
