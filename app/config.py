...
if not TRANSLATION_FILE.endswith('.json'):
    print(f"WARNING: TRANSLATION_FILE ({TRANSLATION_FILE}) should have .json extension", file=sys.stderr)
if not (1024 <= MAX_TRANSLATION_FILE_SIZE <= 50 * 1024 * 1024):
    print(f"WARNING: MAX_TRANSLATION_FILE_SIZE ({MAX_TRANSLATION_FILE_SIZE}) outside recommended range 1KB-50MB", file=sys.stderr)