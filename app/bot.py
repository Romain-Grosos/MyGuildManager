    ...
    def filter(self, record):
        ...
        if ctx and hasattr(ctx, 'guild'):
            if ctx.guild:
                ...
                record.guild_id = f"dm_user_{ctx.author.id}" if hasattr(ctx, 'author') else "dm_unknown"
            
            if hasattr(ctx, 'command') and ctx.command:
                if hasattr(ctx.command, 'parent') and ctx.command.parent:
                    record.command_name = f"{ctx.command.parent.name}/{ctx.command.name}"
                else:
...
class JSONFormatter(logging.Formatter):
    ...
    def format(self, record):
        ...
        if record.exc_info and (config.DEBUG and not getattr(config, 'PRODUCTION', False)):
...
try:
    ...
    logging.warning(f"[Bot] Cannot create rotating log file: {e}, falling back to rotating handler")
    try:
        ...
        logging.warning("[Bot] Cannot create any file handler, using console only")

if getattr(config, 'LOG_FORMAT_JSON', False) and getattr(config, 'LOG_JSON_CONSOLE', False):
...
def _global_exception_hook(exc_type, exc_value, exc_tb):
    ...
    if config.DEBUG and not getattr(config, 'PRODUCTION', False):
        ...
        logging.critical(f"UNCAUGHT EXCEPTION: {exc_type.__name__}: {exc_value}")
...
class BotOptimizer:
    ...
    def __init__(self, bot):
        ...
        self._cold_start_until = time.monotonic() + getattr(config, 'COLD_START_SECONDS', 300)
    ...
    async def get_member_optimized(self, guild, member_id: int):
        ...
        try:
            ...
            logging.warning(f"[BotOptimizer] Member {member_id} not found or forbidden, purged from cache: {e}")
            return None
    ...
    async def get_channel_optimized(self, channel_id: int):
        ...
        try:
            ...
            self._cache_times.pop(key, None)
            logging.warning(f"[BotOptimizer] Channel {channel_id} not found or forbidden, purged from cache: {e}")
            return None
        except Exception as e:
    ...
    def _check_performance_alerts(self):
        ...
        now = time.monotonic()
        recent_window = [p for p in self._performance_window if now - p['timestamp'] <= 900]
        ...
        if fast_pct < fast_threshold:
            ...
            if now - self._alert_cooldown.get(alert_key, 0) > cooldown_seconds:
                logging.warning(f"[Alert] Fast response rate dropped to {fast_pct:.1f}% (threshold: {fast_threshold}%)")
                self._alert_cooldown[alert_key] = int(now)

        if slow_pct > slow_threshold:
            alert_key = 'slow_spike'
            if now - self._alert_cooldown.get(alert_key, 0) > cooldown_seconds:
                logging.warning(f"[Alert] Slow response rate spiked to {slow_pct:.1f}% (threshold: {slow_threshold}%)")
                self._alert_cooldown[alert_key] = int(now)
    ...
    def get_performance_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'uptime_hours': (time.monotonic() - getattr(self.bot, '_start_time_monotonic', time.monotonic())) / 3600,
    ...
    def _calculate_slo_availability(self) -> float:
        ...
        total_ops = self.metrics['commands_executed'] + self.metrics['rate_limit_denials']
        if total_ops == 0:
            return 100.0
    ...
    def check_health_alerts(self):
        ...
        if len(self._member_cache) == 0 and self.metrics['commands_executed'] > 10:
            ...
            if now - alert_info['last_alert'] > alert_info['cooldown']:
                logging.warning("[Alert] Cache appears not loaded despite command activity")
        ...
        if hasattr(self.bot, '_reconnection_count'):
            ...
            if reconnections > getattr(config, 'ALERT_RECONNECTION_THRESHOLD', 5):
                ...
                alert_info = self._health_alerts[alert_key]
                if now - alert_info['last_alert'] > alert_info['cooldown']:
                    logging.warning(f"[Alert] High reconnection count: {reconnections}")
                    alert_info['last_alert'] = int(now)
        ...
        if hasattr(self.bot, '_watchdog_alert_triggered'):
            if self.bot._watchdog_alert_triggered:
                ...
                alert_info = self._health_alerts[alert_key]
                if now - alert_info['last_alert'] > alert_info['cooldown']:
                    logging.warning("[Alert] Watchdog detected slow/stuck tasks")
                    alert_info['last_alert'] = int(now)
    ...
    def track_correlation_id(self, correlation_id: str):
        ...
        if correlation_id in self._correlation_ids_seen:
            ...
            if self._correlation_collision_count % 10 == 0:
                logging.warning(f"[Alert] Correlation ID collisions detected: {self._correlation_collision_count} total")
        else:
        ...
        if execution_time > 100:
            ...
            if config.DEBUG and not getattr(config, 'PRODUCTION', False):
                ...
                if ctx:
                    ...
                    user_info = f"user={ctx.author.id if hasattr(ctx, 'author') else 'unknown'}"
                    context_info = f" | {guild_info} | {user_info}"
            query_safe = query.replace('%s', '?').replace('%d', '?')
            query_preview = query_safe[:100] + "..." if len(query_safe) > 100 else query_safe
            logging.warning(f"[DB] Slow query ({execution_time:.0f}ms){context_info}: {query_preview}")
...
def handle_async_exception(loop, context):
    ...
    if exception:
        logging.error(f"[Bot] Uncaught async exception: {exception}", exc_info=exception)
    else:
        logging.error(f"[Bot] Uncaught async exception: {context['message']}")
...
def validate_config() -> None:
    ...
    for attr, expected_type in required_attrs.items():
        if not hasattr(config, attr):
            ...
            if not isinstance(value, expected_type):
                type_errors.append(f"{attr} should be {expected_type.__name__ if isinstance(expected_type, type) else expected_type}, got {type(value).__name__}")
    
    if missing:
        logging.critical(f"[Bot] Missing required config attributes: {missing}")
        raise SystemExit(1)
    
    if type_errors:
        logging.critical(f"[Bot] Config type errors: {type_errors}")
        raise SystemExit(1)

    if hasattr(config, 'MAX_MEMORY_MB') and config.MAX_MEMORY_MB < 100:
        logging.warning("[Bot] MAX_MEMORY_MB is very low (<100MB), may cause issues")
    
    if hasattr(config, 'MAX_CPU_PERCENT') and config.MAX_CPU_PERCENT < 10:
        logging.warning("[Bot] MAX_CPU_PERCENT is very low (<10%), may trigger false alarms")
    
    if hasattr(config, 'CACHE_TTL_SECONDS') and config.CACHE_TTL_SECONDS < 60:
        logging.warning("[Bot] CACHE_TTL_SECONDS is very low (<60s), may reduce cache efficiency")
    ...
    if rate_limit_window < 10:
        logging.warning("[Bot] RATE_LIMIT_WINDOW_SECONDS is very low (<10s)")
    
    if rate_limit_max < 10:
        logging.warning("[Bot] RATE_LIMIT_COMMANDS_PER_GUILD_PER_MINUTE is very restrictive (<10)")
    
    logging.info("[Bot] Configuration validation completed")
...
def validate_token():
    ...
    token = config.TOKEN
    if not token:
        logging.critical("[Bot] Missing Discord token")
        raise SystemExit(1)

    if not isinstance(token, str) or len(token) < 50:
        logging.critical("[Bot] Invalid Discord token format - too short")
        raise SystemExit(1)

    if token.strip() != token:
        logging.critical("[Bot] Invalid Discord token format - contains whitespace")
        raise SystemExit(1)

    if token.count('.') < 2:
        logging.critical("[Bot] Invalid Discord token format - missing required structure")
        raise SystemExit(1)

    if token.lower() in ['your_token_here', 'bot_token', 'discord_token']:
        logging.critical("[Bot] Placeholder token detected - please use a real Discord bot token")
        raise SystemExit(1)
...
current_command_context: ContextVar[Any] = ContextVar('current_command_context', default=None)
...
def register_background_task(task: asyncio.Task, task_name: str = "unknown") -> None:
    ...
    logging.debug(f"[Bot] Registered background task: {task_name}")
...
async def task_watchdog():
    ...
    try:
            ...
            if hasattr(bot, '_background_tasks'):
                ...
                for task in bot._background_tasks:
                    ...
                    if time_since_yield > watchdog_threshold:
                        logging.warning(f"[Watchdog] Task '{task_name}' hasn't yielded for {time_since_yield:.1f}s - possible deadlock")
        ...
        logging.debug("[Watchdog] Task watchdog cancelled")
        raise
...
def create_command_groups(bot: discord.Bot) -> None:
    ...
    bot.admin_group = discord.SlashCommandGroup(
        ...
        description=GROUPS_DATA.get("admin_bot", {}).get("description", {}).get("en-US", "Bot administration commands"),
        name_localizations=GROUPS_DATA.get("admin_bot", {}).get("name", {}),
        description_localizations=GROUPS_DATA.get("admin_bot", {}).get("description", {}),
        default_member_permissions=discord.Permissions(administrator=True)
    )

    bot.absence_group = discord.SlashCommandGroup(
        name=GROUPS_DATA.get("absence", {}).get("name", {}).get("en-US", "absence"),
        description=GROUPS_DATA.get("absence", {}).get("description", {}).get("en-US", "Manage member absence status"),
    ...
    bot.loot_group = discord.SlashCommandGroup(
        ...
        description=GROUPS_DATA.get("loot", {}).get("description", {}).get("en-US", "Epic T2 loot wishlist management"),
    ...
    bot.staff_group = discord.SlashCommandGroup(
        ...
        description=GROUPS_DATA.get("staff", {}).get("description", {}).get("en-US", "Staff management commands"),
    ...
    bot.events_group = discord.SlashCommandGroup(
        ...
        description=GROUPS_DATA.get("events", {}).get("description", {}).get("en-US", "Guild event management"),
    ...
    bot.statics_group = discord.SlashCommandGroup(
        ...
        description=GROUPS_DATA.get("statics", {}).get("description", {}).get("en-US", "Static group management"),
...
def setup_global_group_error_handlers(bot: discord.Bot) -> None:
    ...
    async def global_group_error_handler(ctx: discord.ApplicationContext, error: Exception):
        ...
        command_info = ctx.command.name if hasattr(ctx, 'command') and ctx.command else 'None'
        guild_info = ctx.guild.id if ctx.guild else 'None'
        logging.error(f"[Bot] GLOBAL_ERROR_HANDLER - Error: {type(error).__name__}: {error} | Guild: {guild_info} | Command: {command_info}")
        ...
        if config.DEBUG and not getattr(config, 'PRODUCTION', False):
            ...
            logging.error(f"[Bot] Error in {group_name}/{command_name} command: {error}")
...
def load_extensions():
    ...
    if hasattr(bot, '_extensions_loaded') and bot._extensions_loaded:
        logging.debug("[Bot] Extensions already loaded, skipping")
        return
...
async def global_rate_limit(ctx):
    ...
    logging.info(f"[Command] Started: {ctx.command.name if ctx.command else 'unknown'}")
    ...
    if not hasattr(bot.guild_command_cooldowns[guild_key], 'popleft'):
        ...
        while (bot.guild_command_cooldowns[guild_key] and 
               bot.guild_command_cooldowns[guild_key][0] < cutoff):
    ...
    if bot._rate_limit_counter % 5000 == 0:
        inactive_threshold = now - getattr(config, 'RATE_LIMIT_PURGE_INACTIVE_SECONDS', 3600)
        inactive_guilds = []
        ...
        if inactive_guilds:
            logging.debug(f"[Bot] Purged {len(inactive_guilds)} inactive guilds from rate limiter")

        if hasattr(bot, 'optimizer'):
            bot.optimizer.check_health_alerts()

    max_commands_per_guild = getattr(config, 'RATE_LIMIT_COMMANDS_PER_GUILD_PER_MINUTE', 50)
    if len(bot.guild_command_cooldowns[guild_key]) >= max_commands_per_guild:
        logging.warning(f"[Bot] Guild rate limit exceeded for {guild_id} ({len(bot.guild_command_cooldowns[guild_key])} commands/min)")
        if hasattr(bot, 'optimizer'):
...
async def clear_context(ctx):
    ...
    logging.info(f"[Command] Completed: {ctx.command.name if ctx.command else 'unknown'}")
...
async def on_application_command_error(ctx: discord.ApplicationContext, error: discord.DiscordException):
    ...
    if isinstance(error, RateLimitExceeded):
        try:
            await ctx.respond("⏰ Too many commands! Please wait a minute before trying again.", ephemeral=True)
        except Exception:
            pass
        return
...
async def on_ready() -> None:
    ...
    if not hasattr(bot, 'http_session'):
        ...
        logging.debug(f"[Bot] Created HTTP session: total={timeout_total}s, connections={limit_total}, IPv4-only={getattr(config, 'HTTP_FORCE_IPV4', False)}")
    ...
    if not hasattr(bot, '_cache_load_attempted'):
        ...
        if not cache_loaded:
            ...
            logging.error("[Bot] Cache loading failed after all retries - bot will run with limited functionality")
    
    if PSUTIL_AVAILABLE and not hasattr(bot, '_monitor_task'):
        bot._monitor_task = asyncio.create_task(monitor_resources(), name="resource_monitoring")
    ...
    if not hasattr(bot, '_optimization_setup_done'):
        ...
        if cache_task:
            ...
            logging.warning("[Bot] Cache maintenance task not started (returned None)")
            
        if cleanup_task:
            bot.register_background_task(cleanup_task, "rate_limit_cleanup")
        else:
            logging.warning("[Bot] Cleanup task not started (returned None)")

        async def periodic_rate_limit_cleanup():
            try:
                while True:
                    await asyncio.sleep(getattr(config, 'RATE_LIMIT_CLEANUP_INTERVAL_SECONDS', 1800))
                    if hasattr(bot, 'guild_command_cooldowns'):
                        ...
                        for g_key in inactive_guilds:
                            ...
                            bot.guild_last_activity.pop(g_key, None)
                        if inactive_guilds:
                            logging.debug(f"[Bot] Periodic cleanup: purged {len(inactive_guilds)} inactive guilds")
            except asyncio.CancelledError:
                logging.debug("[Bot] Periodic cleanup task cancelled")
                raise
        
        periodic_cleanup = asyncio.create_task(periodic_rate_limit_cleanup(), name="periodic_rate_limit_cleanup")
        ...
        if getattr(config, 'TASK_WATCHDOG_ENABLED', True):
            ...
            logging.debug("[Bot] Started task watchdog for deadlock detection")
...
async def performance_stats(ctx):
    ...
    fast_pct = round(100 * (histogram['<=50ms'] + histogram['<=100ms']) / max(1, stats['commands_executed']), 1)
    slow_pct = round(100 * (histogram['<=2s'] + histogram['<=5s'] + histogram['<=10s'] + histogram['>10s']) / max(1, stats['commands_executed']), 1)
...
def setup_tracemalloc():
    ...
    if config.DEBUG and getattr(config, 'TRACEMALLOC_ENABLED', False):
        ...
        logging.info("[Bot] tracemalloc started for memory debugging")
        ...
        def dump_memory_snapshot(signum, frame):
            try:
                ...
                logging.info("[Memory] Top 10 memory allocations:")
                for index, stat in enumerate(top_stats, 1):
                    logging.info(f"[Memory] #{index}: {stat}")

                current, peak = tracemalloc.get_traced_memory()
                logging.info(f"[Memory] Current: {current / 1024 / 1024:.1f}MB, Peak: {peak / 1024 / 1024:.1f}MB")
                
            except Exception as e:
                logging.error(f"[Memory] Error taking snapshot: {e}")
        
        if hasattr(signal, 'SIGUSR1'):
            signal.signal(signal.SIGUSR1, dump_memory_snapshot)
            logging.info("[Bot] SIGUSR1 handler setup for memory snapshots (Unix)")
        elif hasattr(signal, 'SIGBREAK'):
            signal.signal(signal.SIGBREAK, dump_memory_snapshot)
            logging.info("[Bot] SIGBREAK handler setup for memory snapshots (Windows)")
        else:
            logging.warning("[Bot] No suitable signal available for memory snapshots")
    ...
    try:
        while retry_count < max_retries:
            try:
                ...
                logging.info("[Bot] Bot startup cancelled")
                break
                ...
                logging.exception(f"Network error (attempt {retry_count}/{max_retries}) - retrying in {wait_time:.1f}s")
                if retry_count >= max_retries:
        ...
        logging.info("[Bot] Performing cleanup on shutdown")
...
async def cleanup_background_tasks():
    ...
    if hasattr(bot, '_background_tasks'):
        ...
        if bot._background_tasks:
            try:
                ...
                logging.warning(f"[Bot] Background tasks cleanup timed out after {shutdown_timeout}s, forcing shutdown")
                for task in bot._background_tasks:
                    if not task.done():
                        task.cancel()
    ...
    if hasattr(bot, 'http_session') and bot.http_session:
        try:
            ...
            logging.debug("[Bot] Closed shared HTTP session and connector")
        except asyncio.TimeoutError:
            logging.warning("[Bot] HTTP session/connector close timed out, continuing shutdown")
        except Exception as e:
...
def _graceful_exit(sig_name):
    ...
    """
    logging.warning("[Bot] Signal %s received - initiating graceful shutdown", sig_name)
    ...
    async def shutdown():
        try:
            ...
            if not bot.is_closed():
                await bot.close()
            logging.info("[Bot] Graceful shutdown completed")
        except Exception as e:
            logging.error(f"[Bot] Error during shutdown: {e}", exc_info=True)
    ...
    try:
        ...
        try:
            ...
            logging.critical(f"[Bot] Failed to shutdown gracefully: {e}")
            os._exit(1)
...
if __name__ == "__main__":
    ...
    for sig in signals_to_handle:
        try:
            ...
            logging.debug(f"[Bot] Registered signal handler for {sig.name} via loop")
        except (NotImplementedError, AttributeError):
            signal.signal(sig, lambda signum, frame: _graceful_exit(signal.Signals(signum).name))
            logging.debug(f"[Bot] Registered signal handler for {sig.name} via signal module")
